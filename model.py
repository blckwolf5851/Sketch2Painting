...
from keras.layers import MaxPooling2D, Input, Dense, Reshape, UpSampling2D, Concatenate, Conv2D, Activation, Conv2DTranspose, LeakyReLU, Dropout, BatchNormalization, concatenate
...
def load_images(src_path='src_images/', tar_path='tar_images/', type='train', size=(512, 512)):
  ...
  for filename in tqdm(os.listdir(type + src_path)):
  ...
  for filename in tqdm(os.listdir(type + tar_path)):
    # load and resize the image
...
def define_discriminator(image_shape=(512,512,3)):
  ...
  # new_conv = Conv2D(filters=64,
  # 									kernel_size=(3, 3),
  # 									padding='same', kernel_initializer=init)(new_conv)
  ...
  # color_conv = Conv2D(filters=512,
  # 									kernel_size=(3, 3),
  # 									padding='same', kernel_initializer=init)(in_target_image)
  # color_conv = MaxPooling2D(strides=8)(color_conv)
  # color_conv = BatchNormalization()(color_conv)
  # color_conv = LeakyReLU(alpha=0.2)(color_conv)
  # d = Concatenate()([d, color_conv])
  ...
  # keep only up to block4 conv 2 layers, otherwise loss too much structure info


  # model.layers[3].summary()
  # model.summary()
  # compile model
  opt = Adam(lr=0.00035, beta_1=0.5)  # TODO
...
def define_encoder_block(layer_in, n_filters, batchnorm=True):
  ...
  return g
  # vgg_model = VGG16(weights='imagenet', include_top=False)(layer_in)
  # for i in range(13):
  # 	vgg_model.layers.pop()
  # # conditionally add batch normalization
  # if batchnorm:
  # 	vgg_model = BatchNormalization()(vgg_model, training=True)
  # # leaky relu activation
  # vgg_model = LeakyReLU(alpha=0.2)(vgg_model)
  # return vgg_model
...
def define_gan(g_model, d_model, image_shape=(512,512,3)):
  ...
  # compile model
  opt = Adam(lr=0.00025, beta_1=0.5)  # TODO
  model.compile(loss=['binary_crossentropy', 'mae'], optimizer=opt, loss_weights=[1,100])
  return model


def load_real_samples(filename, p=0.95):
  ...
  n = int(p * X1.shape[0])  # sample 20% of the data for training
  sample_inds = np.random.choice(X1.shape[0], n, replace=False)
  X1 = X1[sample_inds]
  X2 = X2[sample_inds]
  del sample_inds
  X1 = (X1 - 127.5) / 127.5
...
def generate(g_model, src_path, tar_path):
  # preprocess(512,512,type='usage')
  for filename in os.listdir(src_path):
    sketch = Image.open(src_path+filename)
    ...
    io.imsave(tar_path+filename, transformed[0])
...
# data = load_real_samples('train_512.npz', p=0.2)
# # data = np.ones((1,512,512,3))
# print('Loaded Train Source Images: ', data[0].shape, '|| Loaded Train Target Images: ', data[1].shape)
# print("...Load Test Data...")
# test_data = load_real_samples('test_512.npz', p=1)
# # test_data = np.ones((1,512,512,3))
...
# ax1.imshow(src_images[id], cmap='gray')
# ax2.imshow(tar_images[id], cmap='gray')
# plt.show()
# del src_images
# del tar_images
...
# start_iter = 1530
try:
  filename = "model_%06d.h5" % (start_iter)
  g_model.load_weights(filename)
  print("Loaded weights at step:", start_iter)
except:
  pass
# d_model.summary()
# g_model.summary()
...
if mode == 'train':
  ...
  generate(g_model, './usage/src_images/', './usage/tar_images/')
